    ...
    <section className="gradient-hero py-20">
      <div className="container mx-auto px-4 max-w-7xl text-center">
        ...
        <h1 className="text-5xl lg:text-7xl font-black text-white mb-6 leading-tight">
          Buy Smart. <span className="text-green-200">Sell Fair.</span>
        </h1>
        <p className="text-xl text-green-100/90 max-w-2xl mx-auto mb-10">
          5,000+ certified products from 200+ verified suppliers. Anti-counterfeit QR verification,
          competitive bulk pricing, and financing integration.
        </p>
...
export function MarketSearchSection() {
  return (
    <section className="py-10 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-semibold transition-colors ${
                tag === "All Products"
                  ...
                  : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-green-50 dark:hover:bg-green-950/30"
              }`}
            >
              {tag}
            </button>
          ...
          {categories.map(({ name, icon, count, bg }) => (
            <Link href="/register" key={name} className={`${bg} border rounded-2xl p-5 text-center card-hover transition-all`}>
              ...
              {count > 0 ? (
                ...
                <div className="text-xs text-purple-600 dark:text-purple-400 font-semibold">Sell Now</div>
              )}
            </Link>
          ))}
        </div>
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {featuredProducts.map(({ name, price, unit, supplier, rating, reviews, verified, tag }) => (
            <Card key={name} className="overflow-hidden card-hover bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
              <div className="h-40 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/20 relative flex items-center justify-center">
                ...
                {tag && (
                  <div className="absolute top-3 left-3">
                    <Badge variant={tag === "Best Seller" ? "default" : tag === "Price Drop" ? "warning" : tag === "Top Rated" ? "earth" : "info"}>{tag}</Badge>
                  </div>
                )}
                {verified && (
                  <div className="absolute top-3 right-3">
                    <div className="flex items-center gap-1 bg-white dark:bg-slate-900 rounded-full px-2 py-0.5 shadow-sm">
                      ...
                      <span className="text-[10px] font-bold text-green-600 dark:text-green-400">Verified</span>
                    </div>
                  </div>
                )}
              </div>
              <CardContent className="p-4">
                ...
                <div className="flex items-end justify-between">
                  <div>
                    <div className="font-black text-green-600 dark:text-green-400 text-lg">{price}</div>
                    <div className="text-xs text-slate-400">per {unit}</div>
                  </div>
                  <Button size="sm" asChild><Link href="/register">Add to Cart</Link></Button>
                </div>
              </CardContent>
            </Card>
          ))}
...
export function VerifiedSuppliersSection() {
  return (
    <section className="py-20 bg-slate-50 dark:bg-slate-950">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {suppliers.map(({ name, products, rating, sales, badge, location }) => (
            <Card key={name} className="p-5 text-center card-hover bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
              <CardContent className="p-0">
                ...
                <div className="flex justify-around text-center">
                  ...
                  <div>
                    <div className="font-bold text-slate-800 dark:text-slate-200 text-sm flex items-center gap-0.5">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
              Our price comparison engine aggregates real-time pricing from all marketplace
              suppliers. Never overpay for farm inputs again.
            </p>
            <div className="space-y-3">
              {["Real-time price tracking across all suppliers", "Historical price trends for seasonal planning", "Price alerts when your desired products drop", "Bulk purchase savings calculator"].map((f) => (
                <div key={f} className="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-300">
                  <TrendingUp className="w-4 h-4 text-green-600 dark:text-green-400 flex-shrink-0" />{f}
                </div>
              ))}
            </div>
          </div>
...
export function BulkPurchaseSection() {
  return (
    <section className="py-20 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 border-y border-amber-200 dark:border-amber-900/30">
      <div className="container mx-auto px-4 max-w-7xl text-center">
        ...
        <p className="text-slate-500 dark:text-slate-400 max-w-xl mx-auto mb-10">
          Join farmer groups to unlock bulk pricing. Form a cooperative purchase of 50+ bags and save up to 25%.
        </p>
        <div className="grid md:grid-cols-3 gap-6 max-w-3xl mx-auto">
          {[{ qty: "5-19 units", saving: "5% off", icon: "📦" }, { qty: "20-49 units", saving: "12% off", icon: "🚛" }, { qty: "50+ units", saving: "25% off", icon: "🏭" }].map(({ qty, saving, icon }) => (
            <div key={qty} className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-amber-200 dark:border-amber-800 text-center shadow-sm">
              ...
              <div className="text-2xl font-black text-amber-600 dark:text-amber-400 mt-1">{saving}</div>
            </div>
          ))}
        </div>
      </div>
...
export function SellProduceSection() {
  return (
    <section className="py-20 bg-white dark:bg-slate-900">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="bg-gradient-to-br from-green-500 to-green-700 dark:from-green-700 dark:to-green-900 rounded-3xl p-8 text-white">
            ...
            <p className="text-green-200/80 mb-6">
              List your produce directly on the marketplace. Reach restaurants, wholesalers, and processors paying fair market prices.
            </p>
          ...
          <div>
            ...
            <div className="space-y-4">
              {["List unlimited produce listings for free", "Connect with verified buyers paying fair prices", "Integrated M-Pesa & bank payments", "Real-time market price transparency", "Logistics partner network for delivery"].map((f) => (
                <div key={f} className="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-300">
                  <Package className="w-4 h-4 text-green-600 dark:text-green-400 flex-shrink-0" />{f}
                </div>
              ))}
            </div>
          </div>
...
export function SecureTransactionsSection() {
  return (
    <section className="py-20 bg-slate-50 dark:bg-slate-950">
      <div className="container mx-auto px-4 max-w-7xl text-center">
        ...
        <div className="grid sm:grid-cols-3 gap-6 max-w-3xl mx-auto">
          {[
            { icon: Lock, title: "Escrow Payments", desc: "Funds held safely until delivery confirmed" },
            { icon: Truck, title: "Order Tracking", desc: "Real-time delivery status updates" },
            ...
            <div key={title} className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 text-center shadow-sm">
              <div className="w-12 h-12 mx-auto rounded-xl bg-green-100 dark:bg-green-900/40 flex items-center justify-center mb-3">
...
export function AntiCounterfeitSection() {
  return (
    <section className="py-20 bg-white dark:bg-slate-900">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
...
export function MarketFinancingSection() {
  return (
    <section className="py-20 bg-gradient-to-br from-green-50 to-emerald-100 dark:from-slate-950 dark:to-green-950/30">
      <div className="container mx-auto px-4 max-w-7xl text-center">
        <Badge className="mb-4">Purchase Financing</Badge>
        <h2 className="text-4xl font-black text-slate-900 dark:text-white mb-6">Buy Now, Pay at Harvest</h2>
        <p className="text-slate-600 dark:text-slate-300 max-w-2xl mx-auto mb-10">
          Purchase inputs on credit. Repayment is automatically deducted when you sell your harvest
